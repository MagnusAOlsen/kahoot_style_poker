...
export interface EvaluatedHand {
  ...
  export class HandEvaluator {
    ...
    static findStraight(cards: Card[]): Card[] | null {
      ...
      if (sorted.includes(12)) sorted.push(-1); // Treat Ace as 1 for A-2-3-4-5
    ...
    static bestOfSeven(cards: Card[]): EvaluatedHand {
      ...
      evaluated.sort((a, b) => this.compareHands(b, a));
      return evaluated[0];
    }
    ...
    static compareHands(a: EvaluatedHand, b: EvaluatedHand): number {
      ...
      for (let i = 0; i < Math.min(a.kicker.length, b.kicker.length); i++) {
        const diff = this.rankValue(b.kicker[i].rank) - this.rankValue(a.kicker[i].rank);
        if (diff !== 0) return diff;
      }
      return 0;
    }
